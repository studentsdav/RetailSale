...
class _StockIssueScreenState extends State<StockIssueScreen> {
  ...
  Future<void> _loadNextGrn() async {
    try {
      ...
      _indentNo.clear();
      showErrorSnackbar(
        'Stock dispatch numbering is not configured for ${DateFormat('dd-MMM-yyyy').format(_date)}. Please add numbering settings first.',
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return EntryShortcuts(
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
            title: const Text('Stock Dispatch Details'),
            centerTitle: true,
          ),
  ...
  Widget _headerCard() {
    return _card(
      title: 'Stock Dispatch Information',
      child: Wrap(
        spacing: 16,
        runSpacing: 16,
        children: [
          ...
          _field(_indentNo, 'Stock Dispatch No', readOnly: true),
          _dropdown('Stock Dispatch Type', issueTypes, _issueType, (v) {
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Remaining After Stock Dispatch : $_remainingStock',
                  style: TextStyle(
    ...
    return SizedBox(
      ...
      child: Focus(
        ...
        child: TextField(
          ...
          decoration: const InputDecoration(labelText: 'Stock Dispatch Qty'),
  ...
  Future<void> _saveIssue() async {
    ...
    if (_issueType == null) {
      _showMessage("Please select stock dispatch type");
      return;
    }
    ...
    try {
      ...
      _showMessage("Stock dispatch saved successfully");

      final shouldPrint = await showDialog<bool>(
        context: context,
        builder: (_) => AlertDialog(
          title: const Text("Print Stock Dispatch Slip"),
          content: const Text("Do you want to print this stock dispatch slip?"),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text("No"),
