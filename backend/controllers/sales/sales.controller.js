...
async function recordSaleBenefitExpenseEntries({
    ...
    if (normalizedSubscriptionAdjustment > 0) {
        await createLedgerEntry({
            db: req.propertyDb,
            outlet_id: req.user.outlet_id,
            txn_date: header.sale_date,
            transaction_type: 'SALE_SUBSCRIPTION_ADJUSTMENT',
            reference_type: 'SALE',
            reference_id: sale.id,
            reference_no: sale.sale_no,
            party_name: partyName,
            payment_method: paymentMode,
            amount_out: normalizedSubscriptionAdjustment,
            notes: `Subscription advance adjustment booked for sale ${sale.sale_no}`,
            created_by,
            transaction
        });
    }
